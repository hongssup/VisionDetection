//
//  LiveFaceDetectionViewController.swift
//  VisionDetection
//
...
class LiveFaceDetectionViewController: UIViewController {
...
extension LiveFaceDetectionViewController: AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    func handleLandmark(_ eye: VNFaceLandmarkRegion2D, faceBoundingBox: CGRect) {
        let landmarkPath = CGMutablePath()
        let landmarkPathPoints = eye.normalizedPoints
            .map({ eyePoint in
            CGPoint(x: eyePoint.y * faceBoundingBox.height + faceBoundingBox.origin.x,
